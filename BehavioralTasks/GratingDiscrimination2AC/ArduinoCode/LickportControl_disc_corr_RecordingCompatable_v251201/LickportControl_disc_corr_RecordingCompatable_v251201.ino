...
volatile int recordState_prev = LOW;
volatile int recordState_new;
volatile int sensorState_change;
volatile int nXtra = 0;
volatile unsigned long t;
volatile unsigned long rewardTemp;
...
volatile bool leftRewardTrigger = false;
volatile bool rightRewardTrigger = false;
...
const int rewardTime = 120; //how long to open the valve for once reward is trigged in ms
...
void loop() {
  ...
  recordState_new = digitalRead(recordingSyncIn);
  t = millis();
  if (recordState_new == HIGH && recordState_prev == LOW) {
    Serial.print(t);
    Serial.println(" recording Begin!");
  } elseif (recordState_new == LOW && recordState_prev == HIGH) {
    Serial.print(t);
  ...
  if (leftRewardTrigger == true){
      digitalWrite(waterValveConL, LOW);
  if (rewardTemp + rewardTime <= t) {
    digitalWrite(waterValveConL, HIGH);
      leftRewardTrigger = false;
    }}

  if (rightRewardTrigger == true){
      digitalWrite(waterValveConR, LOW);
  if (rewardTemp + rewardTime <= t) {
    digitalWrite(waterValveConR, HIGH);
      rightRewardTrigger = false;
    }}
  sensorStateL_prev = sensorStateL_new;
  sensorStateR_prev = sensorStateR_new;
  recordState_prev = recordState_new;
