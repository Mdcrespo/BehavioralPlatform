%% Circles_Shaping_1_vGitHub.m
% By Michael Crespo @ UB
% mdcrespo@buffalo.edu

% 2025-08-26

% the Circles_Shaping_1_vGitHub is built from running_reward_association_2AFC_v241107
% Requires mouse to lick upon random presentation of go stimulus

function varargout = Circles_Shaping_1_vGitHub(varargin)
% CIRCLES_SHAPING_1_VGITHUB MATLAB code for Circles_Shaping_1_vGitHub.fig
%      CIRCLES_SHAPING_1_VGITHUB, by itself, creates a new CIRCLES_SHAPING_1_VGITHUB or raises the existing
%      singleton*.
%
%      H = CIRCLES_SHAPING_1_VGITHUB returns the handle to a new CIRCLES_SHAPING_1_VGITHUB or the handle to
%      the existing singleton*.
%
%      CIRCLES_SHAPING_1_VGITHUB('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CIRCLES_SHAPING_1_VGITHUB.M with the given input arguments.
%
%      CIRCLES_SHAPING_1_VGITHUB('Property','Value',...) creates a new CIRCLES_SHAPING_1_VGITHUB or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Circles_Shaping_1_vGitHub_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Circles_Shaping_1_vGitHub_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Circles_Shaping_1_vGitHub

% Last Modified by GUIDE v2.5 20-Oct-2025 16:14:27

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Circles_Shaping_1_vGitHub_OpeningFcn, ...
                   'gui_OutputFcn',  @Circles_Shaping_1_vGitHub_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Circles_Shaping_1_vGitHub is made visible.
function Circles_Shaping_1_vGitHub_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output argHs, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Circles_Shaping_1_vGitHub (see VARARGIN)
...
% Choose default command line output for Circles_Shaping_1_vGitHub
clear global
...
global approachCorridorLength circlesImage nogoGratingImage
...
% UIWAIT makes Circles_Shaping_1_vGitHub wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Circles_Shaping_1_vGitHub_OutputFcn(hObject, eventdata, handles) 
...
global biasauto numtrialsbias biasprotectionL biasprotectionR consecutiveL consecutiveR rewardzone autowater 
stoptrial = 0;
KbReleaseWait;
...
tic;
    ...
    if results(handles.currenttrial)== 1 || 2
...
while inGratingCorr
     ...
     if rewardreceived == 1 & rewarded == 0
         timeInTask = t+handles.timeAfterReward;
         rewarded = 1;
     end
